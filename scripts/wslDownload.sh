...
download()
{
    ...
    if [ "$QUALITY" = "insider" ]; then
        REPOSITORY="$REPOSITORY-insiders"
        FULLVERSION="$VERSION-$QUALITY"
    else
        FULLVERSION="$VERSION"
    fi
	...
	if [ -f /etc/alpine-release ]; then
		...
		local_url="https://github.com/$REPOSITORY/releases/download/$FULLVERSION/vscodium-reh-alpine-$PLATFORM-$FULLVERSION.tar.gz"
		...
		printf "Downloading:     ";
		local_url="https://github.com/$REPOSITORY/releases/download/$FULLVERSION/vscodium-reh-linux-$PLATFORM-$FULLVERSION.tar.gz"
		set +x
